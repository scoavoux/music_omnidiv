...
On s'intéresse aux titres écoutés plus de 30 secondes. La distribution apparaît très fortement concentrée: peu de titres sont écoutés souvent, la grande majorité est écoutée peu de fois.
...
```{r seuil_popularite}
count(st, sng_id, sng_pop) %>% 
  group_by(sng_pop) %>% 
  arrange(n) %>% 
  slice(1) %>% 
  select(-sng_id, seuil_piste = n, popularite = sng_pop) %>% 
  left_join(count(st, art_id, art_pop) %>% 
              group_by(art_pop) %>% 
              arrange(n) %>% 
              slice(1) %>% 
