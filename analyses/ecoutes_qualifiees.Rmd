...
Ils écoutent un nombre significativement plus importannt d'artistes différents.
...
```{r passifs_div_genres_boxplot, eval=FALSE}
filter(us, !is.na(passifs)) %>% 
  ggplot(aes(passifs, div_genre)) + 
  geom_boxplot(notch = TRUE)
```

```{r passifs_div_artists_boxplot, eval=FALSE}
filter(us, !is.na(passifs)) %>% 
  ggplot(aes(passifs, div_artists)) + 
  geom_boxplot(notch = TRUE)
```

```{r passifs_div_sng_boxplot, eval=FALSE}
filter(us, !is.na(passifs)) %>% 
