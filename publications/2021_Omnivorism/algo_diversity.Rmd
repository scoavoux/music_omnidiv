...
# 
# ## ATTENTION IL FAUT REFAIRE LES BASES POUR TRIVERSITY (pour calcul de diversité agrégée)
# 
...
```{r aggregate_artist_diversity, eval=FALSE}
...
```{r aggregate_genre_diversity, eval=FALSE}
data.frame(Type = c("Collective", "Mean", "N"),
...
filter(us, !is.na(passifs), passifs != "Mixed (freq. algo. $\\approx$ 0)") %>% 
  ggplot(aes(log(div_artists), fill = passifs)) +
...
select(us, div_artists_stock, div_artists_rec) %>% 
  pivot_longer(everything()) %>% 
  ggplot(aes(name, log(value))) +
...
```{r}
mutate(us, diff = div_artists_rec - div_artists_stock) %>%
  ggplot(aes(log(diff))) +
    geom_histogram() +
    labs(x = "Div artist reco - div artist stock")
```
...
select(us, div_genre_stock, div_genre_rec) %>% 
  pivot_longer(everything()) %>% 
...
```{r}
mutate(us, diff = div_genre_rec - div_genre_stock) %>%
  ggplot(aes(diff)) +
    geom_histogram() +
    labs(x = "Div genre reco - div genre stock")
```
...
x <- select(us, user_id, div_artists_rec, div_artists_stock, nb_guid, nb_stock, fq)
x <- pivot_longer(x, cols = starts_with("div_artists")) %>% 
  mutate(nb = ifelse(name == "div_artist_rec", nb_guid, nb_stock)) %>% 
  select(-nb_guid, -nb_stock) %>% 
  group_by(user_id) %>% 
  filter(!any(is.na(value)))

fe_lm_artdiv <- lmer(value ~ log(nb) + (1 | user_id) + name, data = x)
r.squaredGLMM(fe_lm)
stargazer(fe_lm, type = output_type)
```

```{r}
x <- select(us, user_id, div_genre_rec, div_genre_stock, nb_guid, nb_stock, fq)
x <- pivot_longer(x, cols = starts_with("div_")) %>% 
  mutate(nb = ifelse(name == "div_genre_rec", nb_guid, nb_stock)) %>% 
  select(-nb_guid, -nb_stock) %>% 
  group_by(user_id) %>% 
  filter(!any(is.na(value)))

fe_lm_genrediv <- lmer(value ~ log(nb) + (1 | user_id) + name, data = x)

stargazer(fe_lm, type = output_type)
```

```{r}
x <- select(us, user_id, div_omni_rec, div_omni_stock, nb_guid, nb_stock, fq)
x <- pivot_longer(x, cols = starts_with("div_")) %>% 
  mutate(nb = ifelse(name == "div_omni_rec", nb_guid, nb_stock)) %>% 
  select(-nb_guid, -nb_stock) %>% 
  group_by(user_id) %>% 
  filter(!any(is.na(value)))

fe_lm_omnidiv <- lmer(value ~ log(nb) + (1 | user_id) + name, data = x)
