---
title: "Taste and context. What music streaming log data can tell us about omnivorism"
...
Consensus over omnivorism as one contemporary dominant cultural attitude, though doesn't mean it is the only one (although highbrow snobbery). Difficult to know how much of an historical evolution and how much of a matter of change in both sociological approaches to cultural consumption and in the visibility of popular culture consumption in hig-status classes.

Debate very much focused on tastes. Among the causes for a turn towards omnivorism, Peterson proposes many factors affecting preferences. Only one, growing exposure to popular culture, focuses rather on the availability of culture and consumption contexts.

In this paper, we propose to explore the effect of context on omnivorism. We make use of a novel type of dataset that, to our knowledge, has never been used in sociological researches on cultural consumption, log data from an online music streaming service (researchers in other disciplines, such as economics and computer science, have used similar dataset with other theoretical perspectives). Although such a dataset has limitations, which we highlight, especially its lack of demographic variables, we show that it allows for two advances in the omnivorism debate: first, it allows better, less biased measurement of the balance between practices; second, it allows for the study of consumption contexts. 

<!--
Taste omnivorism and context omnivorism. We do not think that there actually are several types of omnivorism, but rather 
-->

# Music consumption, technologies and omnivorism

Cultural consumption is a boundary marker [@lamont1992a]. Since at least the 19^th^ century and the emergence of an industrial elite, culture has been used to show one's group membership or one's exclusion from a group [@dimaggio1982; @levine1990]. From that time, access to cultural product has become a public issue, at least in some societies. In France, theater professionals pushed for a wider popular access as soon as the late 1800s [@dubois2000] and art museums were conceived as educational tools since their modern inception in the late 18^th^ century [@mcclellan1999; @carrier2006].

The history of cultural access is also a history of technologies. In pre-modern times, the invention of the printing press allowed for the wide diffusion of books, even among the working classes [ROGER CHARTIER; traduit?]. Its industrialization in the 19^th^ century gave rise to popular newspapers and cheap paperback books [@rabinowitz2014]; cinema theater offered cheaper entertainment to working class audiences than live theater [@butsch2000_Themakingof] ; and the radio, recorded music, television, recorded movies all produced hopes of cultural democratization.

The Internet is no exception. Sociologists have noticed a trend towards the digitalization of cultural consumption especially for music and audiovisual content [@granjon2007a]. The cultural industries have been worried about threats to copyrights, but users have quickly adopted the various modes of access the technology offered, from peer-to-peer downloads [@beuscart2002_Lesusagersde;@cooper2001_socialorganizationaudio] to streaming. Nowadays, streaming appears as the dominant mode of access, with [HUGE FIGURES ABOUT THE MARKET]. Hopes of democratization are based on two ideas. First, the Internet reduces the cost of access to cultural contents and widens the catalog of available goods. Apple's *iTunes* lowered the cost of music tracks [REF]. Nowadays, most streaming platform such as *Netflix* or *Spotify* propose unlimited access subscription to films or music albums that would have been more costly twenty years ago. Moreover, niche products are more widely available through the Internet than they used to be. Second, more specifically, the Internet is said to increase the diversity of consumption through lowering the cost of information, and not only consumption [@anderson2006_Thelongtail]. Niche products were not only hard to find physically: they were hard to hear about, since mass media tend to focus on a few top-selling products. The idea is that the Internet moved cultural consumption from a mass production paragidm to personnal recommendation. It is said to offer the tools to tailor suggestions on individual tastes, although social theoreticians propose to talk about "algorithmic individuation" rather than personalization, since recommendations are based on similarities between a user and their peers rather [@prey2017_Nothingpersonalalgorithmic].

How did the shift to digital consumption affected listening practices? 

Lots of micro approaches of materiality [@magaudda2011_Whenmaterialitybites; @nowak2016_ConsumingMusicDigital]. Complementarity of ways of accessing music[@nowak2016_ConsumingMusicDigital;@aguiar2016_Digitalmusicconsumption]. Playlist replacing albums[@hagen2015_Theplaylistexperience]?  Show that album not listend to integrally [@louail2017_Headphoneswire]
...
diversity (exploration in their paper) linked with age, but very high variance
...
We first examine the distribution of plays. It is often claimed that the digitalization of music in general, and streaming in particular, reduces concentration [REF]. Log data have no equivalent for physical consumption. The concentration of consumption has been studied mostly through sales of physical supports such as CDs. There are several problems connected with this. First, these are different actions: buying and listening. Next, these are different objects: sales represent inflows of new music, but consumers can still use old CDs; whereas plays on a streaming service inform us about stocks. Finally, the number of references is very different.

Nevertheless, simple descriptive statistics show a highly skewed distribution of plays. A few artists gather most of the attention. This holds true when examining the number of plays, or the number of distinct users who listened to the artist's songs. The distribution is slightly more skewed for the number of play, showing that stars are not only listened by more people, but also are more listened to overall. The following figure shows the long tail distribution of consumption. The distribution at the song and album level (not shown) follow the same pattern.
...
[^replication_top100]: @bourreau2011_Ladiversiteculturelle also study the proportion of sales that go to top 100 albums. But since the size of the catalog is much higher in our dataset (stock rather than inflow of new releases), this .

```{r, eval = FALSE}
(slice(df, 100)$n_cum * 100) %>% round(1)
(slice(df, 1000)$n_cum * 100) %>% round(1)
...
ggplot(df, aes(type_guid, log(div))) +
  geom_violin()

## Uniquement ceux qui ont employé les trois dispositifs
ggplot(group_by(df, user_id) %>% filter(n() == 3), aes(type_guid, log(div))) +
...
Summary of the findings. Effect of recommendation on diversity; effect of rythms on recommendation.

Limits of the study

+ access to demographics
+ quality of meta variables as proxies for context (streaming and listening.)
+ causality: one may argue that the use of recommendation is determined by a taste for diversity rather than just context => it is taste omnivorism after all. While we cannot prove or disprove this, we think that we bring sufficient quantitatve evidence, correlated with qualitative studies, to warrant examination of the idea that context may be responsible at least in part for omnivorism.

Further research: pair survey data and log data. Use digital turn to sharpen methods for sociology of cultural consumption. Hypothesis: link with rythm => survey on rythm.
